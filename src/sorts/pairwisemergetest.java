...
public class pairwisemergetest extends Sort {
    ...
    public pairwisemergetest(Delays delayOps, Highlights markOps, Reads readOps, Writes writeOps) {
        super(delayOps, markOps, readOps, writeOps);
        
        this.setSortPromptID("pairwisemergetest");
        this.setRunAllID("pairwisemergetest");
        this.setReportSortID("pairwisemergetest");
    ...
    public void runSort(int[] array, int currentLength, int bucketCount) {
        ItPairwiseSortingNetwork iterativepairwise = new ItPairwiseSortingNetwork(Delays, Highlights, Reads, Writes);
        int minRunLen = TimSorting.minRunLength(currentLength);
        
        if (currentLength == minRunLen) {
            iterativepairwise.runSort(array, currentLength, bucketCount);
        }
        else {
            int i = 0;
            for (; i <= (currentLength - minRunLen); i += minRunLen) {
                iterativepairwise.customSort(array, i, i + minRunLen);
            }
            if (i + minRunLen > currentLength) {
                iterativepairwise.customSort(array, i, currentLength);
            }
